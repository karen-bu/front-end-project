...
function generateSummary() {
    ...
    if (quizResponses.planetTemperature === 'cold') {
        ...
        const planetTempMsg = 'is of any temperature - you can adjust, you always do';
        $summaryTemperature.textContent = planetTempMsg;
    }
    ...
    if (quizResponses.planetMass === 'small') {
        ...
        const planetMassMsg = "makes you feel different somehow - it doesn't matter how";
        $summaryMass.textContent = planetMassMsg;
    }
    ...
    if (quizResponses.planetPeriod === 'long') {
        ...
        const planetPeriodMsg = "gives you either more or less time - it's a construct, anyway";
        $summaryPeriod.textContent = planetPeriodMsg;
    }
...
const $previousIconRecommendations = document.querySelector('#suggestions-previous-icon');
const $nextIconRecommendations = document.querySelector('#suggestions-next-icon');
const $previousTextRecommendations = document.querySelector('#suggestions-previous-text');
const $nextTextRecommendations = document.querySelector('#suggestions-next-text');
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function buildSuggestionsPage() {
    const exoplanetData = (await fetchExoplanetData(apiURL));
    if (!exoplanetData)
        throw new Error('exoplanetData does not exist!');
    console.log(exoplanetData);
    ...
    if (exoplanetData.length === 0) {
        ...
        for (let i = 0; i < exoplanetData.length; i++) {
            ...
            $column50PlanetEntry.setAttribute('data-planet-recommendation', String(exoplanetData.indexOf(exoplanetData[i])));
...
function generateSuggestionsPage() {
    ...
    const $disclaimerRecommendations = document.querySelector('#suggestions-disclaimer');
    // suggestions arrows
    const $previousNextRecommendations = document.querySelector('#recommendations-previous-next');
